	...
	<table border="1" style="width: 70%; table-layout: fixed">
		...
		<tr>
			...
			<td rowspan=3 colspan=7>工号: <input type="text"
				name="selectKey.membercodeInput" id="membercodeInput" maxlength=8
				value="${selectKey.membercodeInput}" onkeyup="onInput();"/></td>
			<td rowspan=3 colspan=6 align="center" valign="middle"><img
				src="img/search1.jpg" style="width: 100px; height: 60px"></td>
		</tr>
		<tr>
		</tr>
		<tr>
		...
		<tr>
			...
			<td rowspan=4 colspan=5 valign="middle" height="88px">&nbsp; <input
				type="radio" name="selectKey.deptName" id="deptName" value="0"
				checked <c:if test="${selectKey.deptName eq 0}">checked</c:if>>部门名<br />
			&nbsp; <input type="radio" name="selectKey.deptName" id="workerName"
				value="1" <c:if test="${selectKey.deptName eq 1}">checked</c:if>>员工姓名</td>
			<td rowspan=3 colspan=7 valign="middle"><input type="text"
				style="width: 98%" maxlength=40 name="selectKey.nameInput"
				id="nameInput" value="${selectKey.nameInput}" /></td>
