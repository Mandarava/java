...
public abstract class Scan {
	...
	protected Vertical vertical = new Vertical(a, b, map);
	protected Horizon horizon = new Horizon(a, b,map);
	...
	public void scan() {
		...
		// 检测a点,b点的右侧是否能够垂直直连
		for (int y = a.y; y < mainFrame.ROW; y++)
		...
		// 检测a点,b点的下侧是否能够水平直连
		for (int x = a.x; x < mainFrame.COLUMN; x++)
			if (map[x][a.y] == -1 && map[x][b.y] == -1
					&& horizon.horizon(new Point(x, a.y), new Point(x, b.y)))
				linklist.add(new Line(1, new Point(x, a.y), new Point(x, b.y)));
